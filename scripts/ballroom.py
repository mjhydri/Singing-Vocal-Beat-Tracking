...
class BALLROOM(DATA_EXTRACTOR):  # In order to user get_names it uses data handler as parent
    def __init__(self, base_dir=None, splits=None, data_proc=[CQT()]):
        ...
        if base_dir is None:
            base_dir = os.path.join("D:\datasets/ballroom")
        else:
            base_dir = os.path.join(base_dir, "ballroom")
        ...
        for spl in splits:
            ...
            for track in self.tracks[spl][0]:   # covers all tracks in splits:
                ...
                label_path = os.path.join(self.base_dir, 'annotations', f'{track_name}.beats')
    ...
    def get_tracks(self, split):
        ...
        for track in os.listdir(split_path):
            if track.endswith(".wav"):
                tracks.append("BALLROOM#" + split + "#" + os.path.splitext(track)[0])
    ...
    def available_splits():
        ...
        splits = ['ChaChaCha', 'Jive', 'Quickstep', 'Rumba-American',
                  'Rumba-International', 'Rumba-Misc', 'Samba', 'Tango', 'VienneseWaltz', 'Waltz']
