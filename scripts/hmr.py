...
class HMR(DATA_EXTRACTOR):  # In order to user get_names it uses data handler as parent
    def __init__(self, base_dir=None, splits=None, data_proc=[CQT()]):
        ...
        if base_dir is None:
            base_dir = os.path.join("D:/datasets/hmr")
        else:
            base_dir = os.path.join(base_dir, "hmr")
    ...
    def get_tracks(self, split):
        ...
        for track in os.listdir(split_path):
            if track.endswith(".wav"):
                tracks.append("HMR#" + split + "#" + os.path.splitext(track)[0])
