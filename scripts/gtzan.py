...
class GTZAN(DATA_EXTRACTOR):  # In order to user get_names it uses data handler as parent
    def __init__(self, base_dir=None, splits=None, data_proc=[CQT()]):
        ...
        if base_dir is None:
            base_dir = os.path.join("D:/datasets\gtzan")
        else:
            base_dir = os.path.join(base_dir + "gtzan")
        ...
        #     sh(tracks)
        # self.tracks = tracks
        # if train_test_ratio == "all_for_train":
        #     self.train_set = tracks.copy()
        # elif train_test_ratio == "all_for_test":
        #     self.test_set = tracks.copy()
        # else:
        #     self.train_set = tracks.copy()[:int(self.tracks.__len__() * self.train_test_ratio)]
        #     self.test_set = tracks.copy()[int(self.tracks.__len__() * self.train_test_ratio):]
        #
        ...
        for spl in splits:
            for track in self.tracks[spl][0]:  # covers all tracks in splits:
                ...
                jam_path = os.path.join(self.base_dir, 'annotations', 'beat_labels', 'jams', f'{track_name}.wav.jams')
                ...
                beats = np.array([beat.time for beat in beat_data.data])
                downs = np.array([beat.time for beat in down_data.data])
                ...
                if sum(gt[0]) > 4 and sum(gt[1]) > 4:  # since in GTZAN a handful of data do not contain labels
                ...
                with open(self.dir + track, 'wb') as f:
                    pickle.dump(data, f)
        # with open(self.base_dir + "/tracks_list", 'wb') as fa:  # writes exception files
    ...
    def get_tracks(self, split):
        ...
        for track in os.listdir(split_path):
            if track.endswith(".wav"):
                tracks.append("GTZAN#" + split + "#" + os.path.splitext(track)[0])
    ...
    def available_splits():
        ...
        splits = ['blues', 'classical', 'country', 'disco',
